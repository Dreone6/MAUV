...
export function LinkedAccountsScreen({ 
  ...
  const handleToggleLink = (accountId: string) => {
    ...
    setAccounts((prev) =>
      prev.map((a) =>
        a.id === accountId ? { ...a, isLinked: !a.isLinked } : a
      )
    );
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6 pt-12 pb-6 border-b border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-xl text-gray-900">Linked Accounts</h1>
          <div className="w-10" />
        </div>
      </div>
      ...
      <div className="px-6 py-6 space-y-3">
        {accounts.map((account) => (
          ...
          >
            ...
            <button
              onClick={() => handleToggleLink(account.id)}
      ...
      <div className="px-6 mt-4">
        <p className="text-sm text-gray-500 text-center">
          Linking your accounts allows you to sign in with multiple methods and sync your data across platforms.
        </p>
      </div>
    </div>
  );
